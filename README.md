...
Файловая система в памяти. Нужна там, где надо показать работу с файлами и
каталогами, не трогая настоящий диск: тесты не оставляют мусора, а поведение
одинаково на любой машине.

Реализованы привычные операции (`mkdirSync`, `touchSync`, `readdirSync`,
`statSync` и другие), включая ошибки: попытка создать каталог внутри файла или
удалить непустой каталог падает так же, как в настоящей системе.
